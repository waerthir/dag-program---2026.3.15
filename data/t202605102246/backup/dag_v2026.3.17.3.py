...
class FileManager:
    ...
    @staticmethod
    def append_to_temp(data: Dict, output_path: str):
        """线程安全的追加写入"""

        temp_path = output_path + ".tmp"

        # 使用锁确保写操作原子性
        with FileManager._write_lock:
            with open(temp_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps(data, ensure_ascii=False) + '\n')
                f.flush()        # 强制将缓冲区数据写入磁盘
                os.fsync(f.fileno()) # 确保操作系统层面的物理写入

    @staticmethod
    def load_from_temp(output_path: str) -> List[Dict]:
        """从临时文件恢复数据"""
        temp_path = output_path + ".tmp"
        results = []
...
def one_json_process(input_json_name: str):
    ...
    # 提取已经成功的 ID 集合 (使用 set 提高查询效率)
    completed_ids = {item["id"] for item in all_graphs}

    # 启动时，除了读取已完成的 JSON，也尝试从 .tmp 文件恢复（防止上次崩溃没来得及汇总）
    temp_results = FileManager.load_from_temp(output_path)
            ...
            for future in as_completed(future_to_id):
                try:
                    ...
                    if result:
                        ...
                        # 调用我们改良后的线程安全写入
                        FileManager.append_to_temp(result, output_path)
                        logging.info(f"任务 [{result['id']}] 已硬链接存盘。")
                except Exception as e:
                    logging.error(f"某个子任务执行出错: {e}")
        
        except KeyboardInterrupt:
            logging.warning("\n检测到用户中断！正在尝试保存当前进度并退出...")
            # 这里不需要特殊处理，因为每一个成功的 result 已经在上面实时 append_to_temp 了
            raise # 重新抛出以终止程序
