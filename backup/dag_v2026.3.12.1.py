...
class Config:
    ...
    JSON_NAME = [
                ...
                # 'output_glm_geography',
                # 'output_qwen_geography',

                # 'missing_output_glm_math',
                # 'missing_output_minicpm_biology', # 跑不完
                # # 'missing_output_minicpm_math',
                # 'missing_output_qwen_biology', # 没结果
                # 'missing_output_qwen_biology2',
                'missing_output_glm_geography',
                 ]
    INPUT_DIR = './output_CoT_1_missing'
    OUTPUT_DIR = "./output_CoT_1_missing_retry"
...
class GraphAgent:
    ...
    def generate_dag(self, user_data: Dict, max_retries: int = 3) -> Optional[str]:
        ...
        # 核心修改：纯文本模型不需要复杂的嵌套列表，标准的 OpenAI/DashScope 格式即可
        messages =[
...
class DataCleaner:
    ...
    def _fix_invalid_escapes(content: str) -> str:
        """
        修复 JSON 字符串中非法的转义字符。
        逻辑：找到所有的 \，如果它后面跟着的不是 JSON 标准定义的 [u, ", \, /, b, f, n, r, t]，
        就将其替换为双反斜杠 \\。
        """
        # 正则解释：
        # \\ : 匹配一个反斜杠
        # (?!["\\/bfnrtu]) : 断言后面不是合法的 JSON 转义字符
        return re.sub(r'\\(?!["\\/bfnrtu])', r'\\\\', content)

    @staticmethod
    def clean_response(content: str) -> Optional[Dict]:
        """洗涤回复，去除 Markdown 标签并转为 JSON"""
        try:
            # 1. 提取 JSON 核心部分（处理 Markdown 和前后废话）
            ...
            # 2. 核心修复：处理非法的反斜杠转义
            # 比如把 "C:\Users" 变成 "C:\\Users"
            fixed_content = DataCleaner._fix_invalid_escapes(clean_content)

            # 3. 解析
            return json.loads(fixed_content)

        except json.JSONDecodeError as e:
            logging.error(f"JSON 语法错误 (位置 {e.pos}): {e.msg}")
            # 如果还是失败，可以尝试更激进的修复库，如 dirtyjson
            try:
                return dirtyjson.loads(clean_content)
            except ImportError:
                return None
        except Exception as e:
...
def process_single_task(agent: GraphAgent, i: int, block: Dict) -> Optional[Dict]:
    """供多线程调用的单个任务处理函数"""
    try:
        logging.info(f"任务 [{i}] 开始处理...")
        user_input = DataProcessor.format_user_input(block)
        result_graph = agent.generate_dag(user_input)
        
        if result_graph:
            cleaned_graph = DataCleaner.clean_response(result_graph)
            if cleaned_graph:
                logging.info(f"任务 [{i}] 处理成功！")
                return {"id": i, "graph": cleaned_graph}
            else:
                logging.error(f"任务 [{i}] JSON清洗失败。")
        else:
            logging.warning(f"任务 [{i}] API未返回有效内容。")
    except Exception as e:
        logging.error(f"任务 [{i}] 发生未捕获异常: {e}")
    return None
