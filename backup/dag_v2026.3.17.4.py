...
def one_json_process(input_json_name: str):
    ...
    with ThreadPoolExecutor(max_workers=Config.MAX_WORKERS) as executor:
        ...
        #         if result:
        #             # 关键修改：每出一个结果，立即写一个独立小文件
        #             FileManager.save_single_chunk(result, input_json_name)
        #             logging.info(f"任务 [{task_id}] 已独立存盘至碎片目录。")
        #     except Exception as e:
        #         logging.error(f"任务 [{task_id}] 发生未捕获异常: {e}")
